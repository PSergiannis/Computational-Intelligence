...
for i in range(datasetLen):
    ...
    datasetdatasetTransformedVector = (datasetVector - np.average(datasetVector)) / (np.std(datasetVector))#Standardization
    #tempVector = (tempVector - np.average(tempVector)) / (np.std(tempVector))#Standardization
    datasetTransformed[i]=datasetdatasetTransformedVector

with open("E:\\nnProject2022\Datasets\\train-data-standardizedByVector.csv","w+",newline="") as csvFile:
...
with open("E:\\nnProject2022\Datasets\\test-data-standardizedByVector.csv","w+",newline="") as csvFile:
    csvWriter = csv.writer(csvFile,delimiter=',')
    csvWriter.writerows(datasetTransformed)